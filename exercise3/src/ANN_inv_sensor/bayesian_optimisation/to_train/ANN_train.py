...
K_FOLDS_REPEATS = 10  # would ideally like more repeats but time is a serious issue!!!!
ETA = 0.1
EPOCHS = 3000
...
def get_avg_kfold_err(training,weights):
    ...
    for _ in range(K_FOLDS_REPEATS):  # do multiple repeats to reduce validation bias
        ...
        for _ in range(K_FOLDS_VALUE):  # do for each of the k folds
            ...
            back_propagate_fast(TrainingSet(X_train,t_train),weights)  # train the network with the training set partition
