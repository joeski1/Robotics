...
def default_laser_check_for_danger(movement, ranges):
    ...
    if is_simulated():
        distance = 1.2
    return not canMoveForward(ranges, distance)
...
class Movement:
    ...
    def __init__(self):
        ...
        rospy.Subscriber('odom', Odometry, self._odometry_callback, queue_size=1)
        rospy.Subscriber('base_scan', LaserScan, self._laser_callback, queue_size=1)
        if not is_simulated(): # not available in simulator
            rospy.Subscriber('sonar', SonarArray, self._sonar_callback, queue_size=1)
