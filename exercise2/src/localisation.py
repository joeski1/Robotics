...
class ParticleFilterLocalisationNode(object):
    def __init__(self):
        ...
        self._last_odometry = None # FIX
        self._latest_odometry = None # FIX
        ...
        self._cloud_publisher = rospy.Publisher("/particlecloud", PoseArray, queue_size=100)
    ...
    def _odometry_callback(self, odometry):
        ...
        the latest laser.
        """
        if self._last_odometry == None: # FIX
            self._last_odometry = odometry # FIX
        self._latest_odometry = odometry # FIX
        if self._initial_pose_received:
            if self._sufficientMovementDetected(self._particle_filter.estimatedpose): # FIX
                self._last_odometry = self._latest_odometry # FIX
    ...
    def _sufficientMovementDetected(self, latest_pose):
        ...
        """
        latest_pose = self._latest_odometry # FIX
        ...
        prev_x = self._last_odometry.pose.pose.position.x # FIX
        prev_y = self._last_odometry.pose.pose.position.y # FIX
        ...
        prev_rot = self._last_odometry.pose.pose.orientation # FIX
