...
def canMoveForward(data, distance):
    ...
    for i in values:
        ...
        #rospy.lrospy.loginfo("Reading "+str(maxReading) + " ~ index "+str((len(data) / values[-1]) * i))
...
def findGreater(data, fLow, fHigh, sLow, sHigh):
    ...
    #rospy.lrospy.loginfo("Front " + str(firstAverage) + "; Back " + str(secondAverage) + " = " + str(firstAverage - secondAverage))
...
def laser_callback(laser_data):
    ...
    if time.time() > stopTime:
        if stopTime > 0.1:
            print("Robot stopped after %d seconds" % (counter * 30))
            stopTime = 0
        return
